...
export default function ShortTermLiquidity() {
  ...
  useEffect(() => {
    ...
    const startStream = () => {
      if (eventSource || !isStreaming) return;
      const API_URL = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080";
      eventSource = new EventSource(`${API_URL}/api/stream/liquidity?ticker=SPY`);

      eventSource.onopen = () => {
        setLogs(prev => [{ time: new Date().toLocaleTimeString(), score: "CONN", regime: "Connected to IEX Gateway" }, ...prev].slice(0, 8));
      };
      ...
      eventSource.onerror = () => {
        setLogs(prev => [{ time: new Date().toLocaleTimeString(), score: "ERR", regime: "Stream Dropped. Reconnect Manually." }, ...prev].slice(0, 8));
        eventSource?.close();
        eventSource = null;
    ...
    const stopStream = () => { 
      if (eventSource) { 
        eventSource.close(); 
        eventSource = null; 
        setLogs(prev => [{ time: new Date().toLocaleTimeString(), score: "STOP", regime: "Connection Terminated" }, ...prev].slice(0, 8));
      } 
    };
  ...
  return (
    <div className="bg-black/20 backdrop-blur-2xl border border-white/10 p-8 lg:p-10 rounded-[2rem] shadow-2xl grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="space-y-8">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-xs text-white/50 font-mono mt-2 uppercase tracking-[0.1em]">IEX Order-Book Velocity (SPY)</p>
          </div>
          <button
        ...
        <div className="bg-white/5 backdrop-blur-md p-8 rounded-2xl border border-white/10 text-center shadow-xl flex flex-col items-center">
          <span className="text-xs font-sans font-bold text-white/70 uppercase tracking-[0.2em] block mb-4">
            Algorithmic Score
          </span>
          ...
          <div
            className={`inline-block mt-6 px-4 py-1.5 rounded-full text-[10px] font-mono font-bold border tracking-widest uppercase ${
      ...
      <div className="bg-white/5 backdrop-blur-md rounded-2xl p-6 border border-white/10 flex flex-col shadow-xl">
        <div className="border-b border-white/10 pb-4 mb-4 flex justify-between items-center">
          <span className="text-xs font-sans text-white/70 uppercase font-bold tracking-[0.2em]">Decision Log</span>
        ...
        <div className="flex-grow space-y-2.5 font-mono text-[10px] overflow-hidden">
          ...
          {logs.map((log, i) => {
            ...
            const isWarning = log.score === "WARN" || log.score === "ERR";
            return (
              <div
                key={i}
